...
graph LR
    ...
    C -->|read/write| D["Volume / Mount"]
    C -->|communicate| E["Network 网络"]
    F["compose.yaml"] -->|orchestrate| C
    F -->|define| D
    F -->|define| E
```
...
```mermaid
flowchart TD
    START["你想用 Docker 做什么？"] --> Q1{"单个服务容器化？"}
    Q1 -- 是 --> A1["§1 基础对象 → §2 Dockerfile"]
    Q1 -- 否 --> Q2{"多服务项目编排？"}
    Q2 -- 是 --> A2["§3 Compose → §4 挂载 → §5 网络"]
    Q2 -- 否 --> Q3{"生产部署 / 安全加固？"}
    Q3 -- 是 --> A3["§6 安全 → §8 项目设置 → §9 三态"]
    Q3 -- 否 --> Q4{"构建优化 / CI 提速？"}
    Q4 -- 是 --> A4["§7 高阶构建"]
    Q4 -- 否 --> Q5{"AI/LLM 项目？"}
    Q5 -- 是 --> A5["§10 AI 项目 Docker"]
    Q5 -- 否 --> A6["§11 反模式 → §12 学习顺序"]
```
