...
1. **`reduction_db` (降噪量)**：计算指定频带内，原始信号与增强信号的能量比值 ($10 \log_{10}(P_{noisy}/P_{enhanced})$)。正值代表噪声被抑制的程度，接近 0dB 代表信号保留。
	- #### "Reduction" 指标的双刃剑
```Python
red = 10.0 * torch.log10(band_pn / band_pe)
```
- **解读风险**：
	1. 如果 `red` 很大（比如 20dB），有两种可能：
	2. **好事**：背景噪声被完美消除。
	3. **坏事**：鸟叫声被模型“误杀”了（模型认为鸟叫是噪声并消除了）。
- **严谨性补救**：你必须结合 `input_snr_est_db` 来看。
	1. 如果 `Reduction` 很大，且 `input_snr_est_db` 也比较高（>5dB），说明是有效降噪。
	2. 如果 `Reduction` 很大，但 `input_snr_est_db` 极低（接近 0 或负数），说明模型可能把信号也切掉了（因为投影分量变小了）。
2. **`enh_contrast_db` (频谱对比度)**：计算增强后信号在频带内的“峰值能量”与“平均能量”之比。对于鸟声（通常是谐波或类纯音）检测，**对比度越高说明降噪效果越好（信号突出，底噪干净）**。 
3. 获取带内信噪比`input_snr_est_db`，在盲估计（Blind Estimation）中最**稳健**（Robust）且在学术界和工业界（如语音增强、源分离任务）通用的逻辑是基于**正交投影分解**（Orthogonal Projection Decomposition）的方法。
### 核心逻辑：最小二乘法投影（Least Squares Projection）
我们不再假设 `Noisy = Enhanced + Noise`（因为这忽略了相位差异和幅度缩放），而是假设**增强后的信号 $\hat{s}$ 代表了信号的方向**。我们将原始带噪信号 $y$ 分解为两个正交分量：
1. **信号分量（Signal Component）**：$y$ 在 $\hat{s}$ 上的投影（即 $y$ 中与 $\hat{s}$ 线性相关的部分）。
2. **噪声分量（Noise Component）**：$y$ 减去信号分量后的残差（即 $y$ 中与 $\hat{s}$ 不相关的部分）。
#### 数学公式
设 $y$ 为带噪信号，$\hat{s}$ 为模型输出的增强信号（向量形式）：
1. **计算最佳缩放因子** $\alpha$：
    我们要找到一个系数 $\alpha$，使得 $\alpha \cdot \hat{s}$ 最接近 $y$。$$\alpha = \frac{\langle y, \hat{s} \rangle}{\langle \hat{s}, \hat{s} \rangle}$$
    (这解决了“响度不匹配”的问题，自动对齐幅度)
2. **分解原始信号**：$$y_{signal} = \alpha \cdot \hat{s}$$$$y_{noise} = y - y_{signal}$$
3. **计算** SNR：$$\text{Estimated SNR} = 10 \log_{10} \left( \frac{\|y_{signal}\|^2}{\|y_{noise}\|^2 + \epsilon} \right)$$
4. **物理意义**：它是“原始带噪信号”中，“与增强信号线性相关的能量”与“剩余能量”的比值。
5. **数值范围**：
    - **> 15 dB**：信号非常清晰，鸟叫声主导，背景噪声很小。
    - **0 ~ 10 dB**：典型环境，能听到鸟叫，但有明显的背景噪声。
    - **< 0 dB**：噪声能量超过了信号能量，属于高噪环境。

6. **潜在局限性**：
    - 如果模型产生**幻觉（Artifacts）**：模型凭空生成了不存在的鸟叫声，投影法会把这些幻觉当作“信号”，导致 SNR 虚高。
    - 如果模型**过杀（Over-suppression）**：模型把鸟叫声的尾音切掉了。投影法会把原始信号里的尾音归类为“噪声”（因为它在增强信号里不存在），导致 SNR 偏低。
尽管有局限性，**投影法（Projection-based）是目前单通道盲估计中最接近“真实物理信噪比”的数学手段**。它比之前的直接能量相减要可靠得多。
- **信噪比（SNR）计算的科学性**：
    - 采用了 **正交投影分解 (Orthogonal Projection)**。这是目前盲估计（Blind Estimation）中最稳健的方法。
    - **抗干扰能力**：代码先进行了去均值（Zero-mean），然后计算最佳投影系数 $\alpha$，这意味着即使模型输出的音量偏小，或者相位有整体偏移，计算出的 SNR 依然准确，不会像“能量减法”那样出现负无穷或虚高。
- **频带能量分析**：
    - 使用 `torch.searchsorted` 配合线性插值计算累积能量分布（CDF），这是寻找频谱百分位（如 5%~95% 频带）的标准数学方法。
    - **降噪量 (Reduction)**：定义为 $10 \log_{10}(P_n / P_e)$，物理意义明确：正值代表噪声被抑制，0 代表能量不变。
