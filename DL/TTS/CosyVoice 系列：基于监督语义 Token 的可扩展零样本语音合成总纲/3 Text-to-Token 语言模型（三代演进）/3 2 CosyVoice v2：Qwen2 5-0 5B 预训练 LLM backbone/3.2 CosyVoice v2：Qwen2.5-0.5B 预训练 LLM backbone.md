...
![[3.2 CosyVoice v2：Qwen2.5-0.5B 预训练 LLM backbone - v2 LM 架构.excalidraw|800]]

### v1 → v2 的三大简化

1. **移除 Text Encoder**：预训练 LLM 内置文本理解能力

2. **移除 Speaker Embedding**：避免 x-vector 的信息泄漏，用 prompt token 传递音色

3. **复用预训练权重**：利用 Qwen2.5-0.5B 的语言建模能力
