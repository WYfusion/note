...
1. 将合成语音输入多模态 LLM
2. LLM 输出自然语言评价（"这段语音清晰自然，但语速偏快，情感表达略显平淡"）
3. 结构化提取评分或直接用于 A/B 偏好判断
...
1. 从参考语音集提取多维特征分布
2. 从合成语音集提取相同特征分布
3. 计算各维度的 Fréchet Distance (FD)
4. 综合加权得到 TTSDS Score
...
⚖️**关键发现**（来自 Seed-TTS Eval 论文）：WER 和 SIM 之间存在显著的 trade-off——过度优化说话人相似度（SIM↑）会牺牲内容准确性（WER↑），反之亦然。
...
2. **分布级指标标准化** — TTSDS/FAD 等将被纳入标准 Benchmark
3. **统一评测基准出现** — 类似 MMLU/HELM 的 TTS 统一排行榜
4. **交互式语音评测** — 评测对话式 TTS 的 turn-taking 和上下文能力
5. **安全评测内嵌化** — 深伪检测和水印验证成为 TTS 发布的必要环节
6. **端到端评测 Agent** — 自动化评测 Pipeline，从推理到报告全自动
...
| **评测需求** | **推荐方法** | **详见页面** |
| --- | --- | --- |
| 快速原型验证 | WER + UTMOS（自动化） | [[5-客观评价指标体系详解]] |
| 论文发表 | WER + SIM + UTMOS + MOS | [[2-主观评价方法详解]] + [[5]] |
| 产品上线 | AB Test + 安全评测 | [[9-可控语音生成与安全评测]] |
| 多语言场景 | 分语言 WER + X-SIM + LID | [[8-多语言与跨语言TTS评测]] |
| Codec 开发 | PESQ + ViSQOL + FAD | [[6-语音编解码器与Tokenizer评测]] |
...
- [[5-客观评价指标体系详解]] — 传统客观指标详细参考
- [[7-前沿TTS模型Benchmark横评]] — 当前模型横向对比数据
- [[9-可控语音生成与安全评测]] — 安全与可控性评测
- [[3-评测框架与工具链]] — 评测框架工程实现
