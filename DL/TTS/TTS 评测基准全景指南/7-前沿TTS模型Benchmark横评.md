...
⚠️长语音（>10 分钟）是检验 TTS 模型**鲁棒性**的核心场景。大多数模型在长语音下出现严重的重复、遗漏或韵律崩塌问题。
...
2. **Flow Matching 替代 Diffusion**：更快的采样速度和更好的质量（F5-TTS、CosyVoice 3）
3. **更低帧率 Tokenizer**：从 75Hz（EnCodec）→ 12Hz（Qwen3-TTS），序列长度压缩 6 倍
4. **端到端可控生成**：通过自然语言指令控制语音属性（InstructTTS、Qwen3-TTS VD）
5. **多语言统一模型**：单一模型支持 10+ 语言的高质量合成
...
2. **Speaker Encoder 差异**：SIM 值依赖于具体的说话人编码器
3. **评测集版本**：Seed-TTS Eval 有多个子集和版本
4. **推理配置**：温度、top-k/top-p 等采样参数影响结果
5. **文本预处理**：标点、数字归一化方式不同会影响 WER
