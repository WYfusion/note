...
🗺️**核心评测问题**：模型在非训练语言上的泛化能力如何？跨语言声音克隆是否保真？方言/口音能否忠实再现？
...
2. **跨语言迁移**（Cross-lingual Transfer）— 用 A 语言的参考音频合成 B 语言的语音
3. **代码混合**（Code-switching）— 句内多语言混合场景
4. **方言与口音**（Dialect & Accent）— 同一语言下的方言/区域口音变体
...
⚠️**关键陷阱**：不同论文使用不同 ASR 后端计算 WER，数值不可直接比较。例如 CosyVoice 3 使用 SenseVoice-Large，而 Seed-TTS 使用 Whisper，同一合成音频的 WER 差异可达 2-5 个百分点。
...
2. **分语言报告**：不要只报平均值，每种语言单独列出指标
3. **跨语言矩阵**：对声音克隆模型，报告 N×N SIM 矩阵
4. **方言单独评测**：方言与标准语分开统计
5. **人工评测兜底**：多语言 MOS 必须由对应语言母语者评测
6. **报告 ASR/编码器版本**：明确标注 Whisper 版本、SIM 编码器类型
...
- [[1-评测数据集与测试集]] — 通用评测数据集总览
- [[5-客观评价指标体系详解]] — WER/SIM/UTMOS 等指标详解
- [[7-前沿TTS模型Benchmark横评]] — 模型横向对比
- [[3-评测框架与工具链]] — 评测工具链与自动化流程
