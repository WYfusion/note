> [📎] 前置知识：[[1.2 VALL-E 类范式（GPT-SoVITS 的“GPT”血统）]]（GPT 血统的概念基础）、[[1.3 自监督语音表征（SSL Features）]]（输入语义来源）、[[1.4 向量量化（VQ）与离散语义 Token]]（离散化）、Decoder-only Transformer、KV Cache、Top-k/Top-p sampling。
...
[[5.1 Text2SemanticDecoder 总结构]]

[[5.2 训练损失与目标]]

[[5.3 推理采样策略]]

[[5.4 PyTorch Lightning 训练壳]]
