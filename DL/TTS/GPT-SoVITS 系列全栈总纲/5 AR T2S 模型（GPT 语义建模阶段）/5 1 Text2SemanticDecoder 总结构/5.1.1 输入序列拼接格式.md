...
两路同 token 数（都是 phoneme 级，因为 BERT 已在文本前端做了「char→phoneme」对齐扩展，见 [[3.3.4 BERT 特征落盘 pickle 格式|3.3.4 BERT 特征落盘 pickle 格式]]）。
...
[[5.1.1.1 phone + bert + prompt 拼接顺序|5.1.1.1 phone + bert + prompt 拼接顺序]]
