> 📎 **前置**：BigVGAN 上采样伪信号；Mel 频谱重建；NSF（Neural Source Filter）；anti-aliased upsample；[[10.4 V3（2025-02，407M，CFM 化）|10.4 V3（2025-02，407M，CFM 化）]]、[[10.5 V4（48k 原生 + 修复金属噪声）|10.5 V4（48k 原生 + 修复金属噪声）]]。
...
Euler 法 $x_{t+\Delta t} = x_t + \Delta t \cdot v_\theta$，局部截断误差 $O(\Delta t^2)$。8 步即 $\Delta t = 0.125$，累计误差与步长平方相关：
