...
> [!important]
> 
> 本页属于 [[1.5.2 Flow Matching 训练目标|1.5.2 Flow Matching 训练目标]]。

---

## 0. 定义

> **Conditional FM 推导** = 「证明 $\nabla_\theta \mathcal L_{\text{FM}} = \nabla_\theta \mathcal L_{\text{CFM}}$」。这个定理让 Flow Matching 从「不可计算」变「可训练」。

---

## 1. 三个量

- **边缘路径** $p_t(x) = \int p_t(x|x_1) p_{\text{data}}(x_1) dx_1$。
...
与 $\mu_t(x_1) = t x_1,\ \sigma_t = 1 - (1-\sigma_{\min})t$。令 $\sigma_{\min} \to 0$：$\mu_t = t x_1,\ \sigma_t = 1-t$。
