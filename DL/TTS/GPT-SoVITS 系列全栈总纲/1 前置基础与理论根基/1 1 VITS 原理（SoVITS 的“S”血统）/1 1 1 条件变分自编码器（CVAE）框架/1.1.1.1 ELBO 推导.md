...
> [!important]
> 
> 本页属于 [[1.1.1 条件变分自编码器（CVAE）框架]] 的「精准定义」层。
...
仅当 $q_phi(z|x) = p_theta(z|x)$（后验完全拟合）时，ELBO = 真实对数似然。**ELBO 越高 ≈ 似然下界越高 + 后验越准**。

---

## 4. 在 VITS / SoVITS 中的体现

$x$ = mel-spectrogram；$z$ = posterior latent；condition $c$ = phonemes + bert。VITS 采用 ELBO + GAN + duration；见 [[1.1.1 条件变分自编码器（CVAE）框架]]。
