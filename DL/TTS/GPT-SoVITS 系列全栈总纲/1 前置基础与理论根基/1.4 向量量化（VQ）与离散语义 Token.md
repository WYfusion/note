...
> [!important]
> 
> 建议先读 [[1.3 自监督语音表征（SSL Features）|1.3 自监督语音表征（SSL Features）]]，VQ 的输入正是 SSL 特征。
...
GPT-SoVITS 的 VQ 模块**内置在 SoVITS 中**（非单独训练），与 SoVITS 解码器一起以 ELBO + GAN 损失训练。AR 头**不参与 VQ 训练**，只使用冻结后的 VQ 输出。详见 [[4.3 SoVITS 内置 VQ Encoder|4.3 SoVITS 内置 VQ Encoder]]。
...
[[1.4.1 VQ-VAE 基础|1.4.1 VQ-VAE 基础]]
...
[[1.4.2 EMA 码本更新|1.4.2 EMA 码本更新]]

1. Yu, J. et al. (2022). _VQ-GAN improved_. ICLR.

[[1.4.3 Codebook 死码与重启|1.4.3 Codebook 死码与重启]]

[[1.4.4 GPT-SoVITS 的 VQ 维度与码本规模|1.4.4 GPT-SoVITS 的 VQ 维度与码本规模]]