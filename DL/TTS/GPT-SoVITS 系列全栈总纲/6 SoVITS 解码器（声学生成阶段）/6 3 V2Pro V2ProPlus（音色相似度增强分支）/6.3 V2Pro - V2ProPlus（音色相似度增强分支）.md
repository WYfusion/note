> [📎] 前置知识：[[1.1 VITS 原理（SoVITS 的“S”血统）]]、说话人验证（Speaker Verification, SV）、ECAPA-TDNN / ERes2Net 家族、Embedding 注入位置（pre / post-prior、FiLM、Adapter）。
...
> [🔧] **误差补偿头是什么？** ProPlus 在主 Decoder 之后接一个小 CNN，输入是 Decoder 输出 + spk_emb，输出一个 **残差波形** $Delta w$，最终 $w = w_{text{main}} + alpha Delta w$。它专门学「主 Decoder 漏掉的音色细节」，类似 boosting。代价是 +30% 参数 +30% 推理时间。
...
[[6.3.1 ERes2NetV2 Speaker Embedding]]

[[6.3.2 SV Embedding 注入位置]]

[[6.3.3 Pro vs ProPlus（规模 - 误差补偿头）]]
