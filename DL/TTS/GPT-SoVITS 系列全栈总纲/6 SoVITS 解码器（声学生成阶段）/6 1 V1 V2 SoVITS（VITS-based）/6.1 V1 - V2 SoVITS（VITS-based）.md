> [📎] 前置知识：[[1.1 VITS 原理（SoVITS 的“S”血统）]]、Posterior / Prior 概念、Normalizing Flow、HiFi-GAN、MPD / MSD、对抗损失、Feature Matching Loss、KL 散度。
...
|损失|形式|权重|
|---|---|---|
|**Mel recon**|$\|text{mel}(hat x) - text{mel}(x)\|
|**KL**|$mathrm{KL}(q(z\|x) \|
|**Adv (G)**|$\mathbb E[(D(\hat x)-1)^2]$|1.0|
|**FM**|$sum_l \|D_l(x) - D_l(hat x)\|
...
[[6.1.1 Prior Encoder（文本+BERT→先验分布）]]

1. GPT-SoVITS source: `GPT_SoVITS/module/models.py`, `module/mrte_model.py`.

[[6.1.2 Posterior Encoder（线性谱→后验分布）]]

[[6.1.3 Flow 模块]]

[[6.1.7 损失函数总览（mel - KL - adv - fm）]]
...
[[6.1.4 Duration Predictor 的取舍]]

[[6.1.5 Decoder（HiFi-GAN 风格生成器）]]
