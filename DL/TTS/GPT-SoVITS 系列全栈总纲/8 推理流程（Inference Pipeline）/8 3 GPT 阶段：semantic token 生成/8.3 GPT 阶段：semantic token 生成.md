> 📎 **前置**：KV Cache；[[5.1 Text2SemanticDecoder 总结构]]；[[5.3 推理采样策略]]；Flash-Attention；speculative decoding。
...
> [💡] **顺序敏感**：rep_penalty 必须在 top_k/p 之前；否则 penalty 被高概率词遮蔽。详见 [[5.3 推理采样策略]]。
