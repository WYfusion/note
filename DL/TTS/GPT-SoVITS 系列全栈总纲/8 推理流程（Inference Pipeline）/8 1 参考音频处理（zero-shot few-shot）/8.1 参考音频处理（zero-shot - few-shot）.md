> 📎 **前置**：[[4.4 prompt_semantic（参考音频→AR prompt）|4.4 prompt_semantic（参考音频→AR prompt）]]；zero-shot ICL；VALL-E prompt 范式；SNR / LUFS 度量。
...
> ⚠️ **多条 ref 拼接 = 不同说话人混淆**：避免。需多情感选用 V2Pro+ 的多参考槽位机制（见 [[9.2 推理 WebUI（inference_webui.py）|9.2 推理 WebUI（inference_webui.py）]]）。
...
[[8.1.1 参考音频质量门槛（信噪比 - 时长）|8.1.1 参考音频质量门槛（信噪比 - 时长）]]

[[8.1.2 prompt_text 必要性与错误代价|8.1.2 prompt_text 必要性与错误代价]]
