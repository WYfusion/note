...
> **定位**：`2-get-hubert-wav32k.py` 是训练数据准备的「SSL 抽特征 + 重采样」阶段。本节拆输入输出、hubert 加载、 为何要同时出 32k wav、与 [[4.1 重采样策略（16k - 24k - 32k - 48k）|4.1 重采样策略（16k - 24k - 32k - 48k）]] 的关系。
...
仅使用决策性层，dropout 关闭。详见 [[4.1.1 librosa - torchaudio Resample 实现差异|4.1.1 librosa - torchaudio Resample 实现差异]]。
...
fp16 存储「未量化的 SSL 特征」能节太 50% 磁盘，听感几乎无损。 后续 [[7.1.3 3-get-semantic.py（VQ 量化 + name2semantic）|7.1.3 3-get-semantic.py（VQ 量化 + name2semantic）]] 读取后走 VQ 量化 不受影响。

## 7. 多 GPU 切片

同 [[7.1.1 1-get-text.py（文本→音素 + BERT）|7.1.1 1-get-text.py（文本→音素 + BERT）]] 一样使用 `--i_part / --all_parts`。 可以与之同跑（互不依赖）。
