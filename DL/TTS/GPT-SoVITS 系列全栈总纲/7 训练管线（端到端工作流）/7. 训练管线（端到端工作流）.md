[[7.2 Stage 1：SoVITS 训练（s2_train.py）]]

> 📎 **前置知识**：熟悉 [[5. AR - T2S 模型（GPT 语义建模阶段）]] 与 [[6. SoVITS 解码器（声学生成阶段）]]；理解 PyTorch Lightning Trainer；了解少样本微调（SFT / LoRA）概念。
...
> ⚠️ **bf16 + grad clip 联用注意**：详见 [[5.4 PyTorch Lightning 训练壳]]，bf16 下 grad_norm 会偏小，clip 阈值需相应放宽。

## 5. 子页面导航

[[7.1 数据准备流水线]]

[[7.3 Stage 2：GPT - AR 训练（s1_train.py）]]

[[7.4 少样本微调（Few-shot SFT）]]
