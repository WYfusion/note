> 📎 **前置**：[[5. AR - T2S 模型（GPT 语义建模阶段）]]；PyTorch Lightning；Warmup+Cosine LR；BucketSampler；teacher forcing。
...
[[7.3.1 BucketSampler（长度分桶）]]

[[7.3.2 学习率 Warmup（v2 外挂补丁）]]

[[7.3.3 半精度训练（precision- 16-mixed）]]
