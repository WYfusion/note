...
核心思路: Qwen3-TTS 是一个多语言、可控、鲁棒的流式文本转语音模型，支持3秒语音克隆和10种语言的生成，首包延迟低至97ms。其核心创新包括双轨道自回归架构和两种互补的语音 Tokenizer，分别针对不同的延迟和质量需求。模型经过三阶段预训练和后训练，能够生成长达10分钟的流畅语音，并在多语言生成和语音克隆任务中表现出色。

解决的问题: 
1) 现有 TTS 缺乏对语音属性的细粒度自然语言控制能力；
2) 超低延迟流式合成与高质量之间的矛盾；
3) 多语言（10+ 语种）零样本语音克隆中的内容一致性和说话人相似度不足；
4) 长语音（>10分钟）合成中的重复、遗漏和韵律断裂问题；
5) 语义 Tokenizer 表达力不足 vs 纯声学 Tokenizer 低级细节过多导致 LLM 建模困难的平衡。

## 实现方法:
### 核心架构：
基于 Qwen3 LM 的双轨道自回归架构，文本和声学 token 沿通道轴拼接进行实时合成。
#### 两种 Tokenizer 路线：
(1) Qwen-TTS-Tokenizer-25Hz — 基于 Qwen2-Audio 的 25Hz 单码本语义-声学融合 Tokenizer，配合 Block-wise DiT + BigVGAN 流式解码；
(2) Qwen-TTS-Tokenizer-12Hz — 12.5Hz 16层多码本 RVQ Tokenizer（WavLM 语义监督 + 对抗训练），配合轻量因果 ConvNet 解码，首包延迟低至 97ms。12Hz 变体引入 MTP（Multi-Token Prediction）模块分层预测残差码本。
...
[Qwen3-TTS 评价指标详解](Qwen3-TTS%20评价指标详解.md)
[Qwen3-TTS 训练数据集与方法原理](Qwen3-TTS%20训练数据集与方法原理.md)
[Qwen3-TTS 模型组件架构细节](Qwen3-TTS%20模型组件架构细节.md)
## 1. 论文概述
...
2. 语音使用 **Qwen-TTS-Tokenizer** 编码
3. 将文本 token 和声学 token 沿**通道轴（channel axis）**拼接
4. 接收到一个文本 token 后，模型**立即预测**对应的声学 token
5. 声学 token 由 **Code2Wav** 模块转换为波形
...
- 采用**分层预测方案（Hierarchical Prediction）**：
    ...
    2. **MTP（Multi-Token Prediction）模块** → 生成所有**残差码本**（第 1-15 层）
...
2. 1.7B → 0.6B scaling 带来一致性提升
3. **12Hz-1.7B 在 test-en 上达到 SOTA**（WER 1.24）
...
2. **双轨道 LM 架构**：文本与声学 token 沿通道轴拼接，实现实时流式合成
3. **MTP 模块**：分层预测多码本序列，单帧即时生成，极大降低延迟
4. **三阶段预训练 + 三阶段后训练**：从通用能力到高质量到长上下文，再经 DPO → GSPO → Speaker SFT
5. **概率激活 Thinking Pattern**：增强复杂指令跟随能力
6. **500 万小时**多语言训练数据，覆盖 10+ 语种
