...
2. 拼接后的向量送入 Transformer 层进行自回归计算
3. 模型**立即预测**当前时间步对应的语音 token
4. 预测的语音 token 被送入解码器生成音频片段
...
[Qwen2-Audio 多模态音频理解模型详解](Qwen2-Audio%20多模态音频理解模型详解.md)
**两阶段训练策略**：
**Stage 1 — 语义对齐**：
...
[DiT（Diffusion Transformer）在语音合成中的应用详解](DiT（Diffusion%20Transformer）在语音合成中的应用详解.md)
...
**阶段 B — BigVGAN 声码器**：
[BigVGAN 通用神经声码器详解](BigVGAN%20通用神经声码器详解.md)
...
[Mimi 音频编解码器架构详解](Mimi%20音频编解码器架构详解.md)
**核心参数**：
...
2. 第 1 层：对残差 $r_1$ 进行量化，得到 $hat{r}_1$，新残差 $r_2 = r_1 - \hat{r}_1$
3. 第 $k$ 层：对 $r_k$ 进行量化……
4. 最终重建：$hat_x = hat_{x_0}+ hat_{r_1} + hat_{r_2} + ... + hat_{r_{15}}$
...
2. **In-Context Learning 克隆**：将参考语音的文本-语音对作为上下文前缀，利用 LM 的 few-shot 能力进行更精确的韵律模仿
...
2. （参考语音 → Speaker Encoder → Speaker Embedding）
3. 文本 token + 前序语音 token → 沿通道轴拼接 → Qwen3 LM（0.6B/1.7B）
4. LM 线性头 → 预测当前帧的**单个**语音 token（25Hz 单码本）
5. 累积 8 个 token → 送入 Chunk-wise DiT（Flow Matching）→ 320ms Mel 频谱图
6. Mel 频谱图 → BigVGAN 声码器 → 音频波形
7. 输出首个音频包（约 190ms 可播放音频）

### 6.2 Qwen3-TTS-12Hz 完整流程
1. 文本 → Qwen Tokenizer → 文本 token
2. （参考语音 → Speaker Encoder → Speaker Embedding）
3. 文本 token + 前序语音 token → 沿通道轴拼接 → Qwen3 LM（0.6B/1.7B）
4. LM 线性头 → 预测当前帧的**第 0 层**码本 token（语义层）
5. LM 隐藏状态 + 第 0 层 token → **MTP 模块** → 并行预测第 1-15 层 RVQ token
6. 16 层 token embeddings 求和 → 因果 ConvNet 解码器 → 80ms 音频波形
7. 输出首个音频包（首包延迟 ~97ms）
...
2. **双轨道通道拼接** → 避免序列膨胀，保持文本-语音对齐，支持流式生成
3. **双 Tokenizer 路线** → 25Hz 优化质量与长语音稳定性，12Hz 优化延迟与效率
4. **MTP 分层预测** → 解决多码本预测的效率瓶颈，实现单帧即时生成
5. **联合训练 Speaker Encoder** → 端到端优化说话人克隆质量
6. **ChatML 统一格式** → 灵活支持多种控制模式，复用 LM 的指令跟随能力