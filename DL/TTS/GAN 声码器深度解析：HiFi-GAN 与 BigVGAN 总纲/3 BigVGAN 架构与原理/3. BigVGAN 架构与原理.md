...
> 
> $\sin^2(x)$ 始终非负（$\geq 0$），加上恒等项 $x$ 后保证了单调性（$f'_\alpha(x) = 1 + \sin(2\alpha x) \geq 0$）。如果用裸的 $\sin(x)$，函数会在局部区间非单调，导致梯度消失/振荡，训练不稳定。$\sin^2$ 是「让神经网络学习周期函数」的关键技术洞察 [Liu et al., 2020]。
...
|**问题**|**现象**|**解决方案**|
|---|---|---|
|早期崩溃|判别器损失在几千步内收敛到 0|学习率降半：$2 \times 10^{-4} \to 1 \times 10^{-4}$|
|模式崩溃|生成器输出单一模式|Batch size 加倍：$16 \to 32$|
