...
![[2026-04-18 00.44.26TTS演进路线.excalidraw|600]]
...
- **LM-based 成为主流**：CosyVoice、Qwen3-TTS、SparkTTS 等均采用 LLM 作为 TTS 骨干

- **Flow Matching 替代 Diffusion**：更快的采样速度和更好的质量（F5-TTS、CosyVoice 3）

- **更低帧率 Tokenizer**：从 75Hz（EnCodec）→ 12Hz（Qwen3-TTS），序列长度压缩 6 倍

- **端到端可控生成**：通过自然语言指令控制语音属性（InstructTTS、Qwen3-TTS VD）

- **多语言统一模型**：单一模型支持 10+ 语言的高质量合成
