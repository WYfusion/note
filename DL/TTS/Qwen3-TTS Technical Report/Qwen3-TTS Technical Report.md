...
核心思路: Qwen3-TTS 是一个多语言、可控、鲁棒的流式文本转语音模型，支持3秒语音克隆和自然语言控制语音属性，具有超低延迟（首包97ms）和长达10分钟的连续合成能力。核心创新包括双轨道自回归架构和两种互补的语音 Tokenizer，分别优化质量和延迟。模型经过三阶段预训练和后训练，使用超过500万小时的多语言数据，展现出在多语言生成和可控语音生成方面的优越性能。
...
> **L2 主章节**
> 
> 1. [[DL/TTS/Qwen3-TTS Technical Report/TTS 评测基准全景指南/Qwen3-TTS 评价指标详解|Qwen3-TTS 评价指标详解]]
> 
> 1. [[DL/TTS/Qwen3-TTS Technical Report/TTS 评测基准全景指南/TTS 评测基准全景指南|TTS 评测基准全景指南]]
> 
> 1. [[DL/TTS/Qwen3-TTS Technical Report/Qwen3-TTS 训练数据集与方法原理/Qwen3-TTS 训练数据集与方法原理|Qwen3-TTS 训练数据集与方法原理]]
> 
> 1. [[DL/TTS/Qwen3-TTS Technical Report/Qwen3-TTS 模型组件架构细节/Qwen3-TTS 模型组件架构细节|Qwen3-TTS 模型组件架构细节]]
> 
> 1. [[Qwen3-TTS 实验结果详解]] —— Seed-TTS / 多语言 / 跨语言 / 可控 / 长语音 全表
> 
> 1. [[Qwen3-TTS 流式效率与部署]] —— 延迟拆解 / vLLM / CUDA Graph
> 
> 1. [[Qwen3-TTS 开源生态与使用指南]] —— HF / 实战代码 / 微调
> 
> **L4 深挖页**
> 
> - [[RVQ 数学原理与训练目标]]（位于 Mimi）
> 
> - [[Flow Matching 数学推导与实现]]（位于 DiT）
> 
> - [[DPO 与 GSPO 对齐公式推导]]（位于 训练方法）
> 
> - [[ChatML 输入 Pipeline 与任务统一]]（位于 训练方法）

[[DL/TTS/Qwen3-TTS Technical Report/TTS 评测基准全景指南/Qwen3-TTS 评价指标详解|Qwen3-TTS 评价指标详解]]

[[DL/TTS/Qwen3-TTS Technical Report/TTS 评测基准全景指南/TTS 评测基准全景指南|TTS 评测基准全景指南]]
...
> [!important]
> 
> 下图展示从文本输入到音频输出的完整双路径数据流，清晰呈现 **25Hz 与 12Hz** 两种推理路径的模块关系与分岐点。

![[2026-04-18 09.17.40Qwen3TTS模型以及变体.excalidraw|600]]
