...
> [!important]
> 
> Qwen3-TTS 涵盖三类核心模块：两种 Tokenizer（上游）、LM 骨干（中间）、两条 Detokenizer 路径（下游）。下图从模块依赖角度统一展示它们的上下游关系。

![[2026-04-18 09.45.12Qwen3TTS三大件.excalidraw|800]]
...
> **依赖关系要点**
> 
> - **Tokenizer ↔ LM**：25Hz 线性头与 12Hz 分层预测共享同一 LM 隐藏状态 $h_t$。
> 
> - **LM ↔ Speaker Encoder**：Speaker embedding 通过双轨道通道注入，影响所有下游节点。
> 
> - **LM ↔ Detokenizer**：仅交接 token 序列与 speaker 条件，两条路径可互换不重训骨干。
> 
> - **跕模块共享**：Qwen2-Audio 的编码能力、Mimi 的 RVQ 设计、BigVGAN 的通用声码器及 DiT 的 Flow Matching 皆为电驻型组件，传统上下文明确。

---

[[DL/TTS/Qwen3-TTS Technical Report/Qwen3-TTS 模型组件架构细节/Qwen2-Audio 多模态音频理解模型详解|Qwen2-Audio 多模态音频理解模型详解]]
...
[[DL/TTS/Qwen3-TTS Technical Report/Qwen3-TTS 模型组件架构细节/DiT（Diffusion Transformer）在语音合成中的应用详解/DiT（Diffusion Transformer）在语音合成中的应用详解|DiT（Diffusion Transformer）在语音合成中的应用详解]]
...
[[DL/TTS/Qwen3-TTS Technical Report/Qwen3-TTS 模型组件架构细节/BigVGAN 通用神经声码器详解|BigVGAN 通用神经声码器详解]]
...
[[DL/TTS/Qwen3-TTS Technical Report/Qwen3-TTS 模型组件架构细节/MTP 模块（Multi-Token Prediction）详解|MTP 模块（Multi-Token Prediction）详解]]
