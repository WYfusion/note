...
- 最早由 Peebles & Xie（2023）在图像生成领域提出（论文：_Scalable Diffusion Models with Transformers_, ICCV 2023）
...
> 
> 1. **可扩展性好**：参数量增大时质量持续提升，符合 scaling law
> 
> 1. **配合 Flow Matching 效率高**：少量去噪步数即可生成高质量 Mel 频谱图
> 
> 1. **易于流式化**：滑动窗口注意力机制天然支持 chunk-wise 生成

---

## 7. 参考文献

- Peebles, W. & Xie, S. (2023). _Scalable Diffusion Models with Transformers_. ICCV 2023. arXiv:2212.09748

- Lipman, Y. et al. (2023). _Flow Matching for Generative Modeling_. ICLR 2023. arXiv:2210.02747

- Qwen3-TTS Technical Report — Chunk-wise DiT 解码器设计部分

- Esser, P. et al. (2024). _Scaling Rectified Flow Transformers for High-Resolution Image Synthesis_（Stable Diffusion 3 技术报告）

## 🔗 DiT 与上下游模块的协作关系

> [!important]
> 
> DiT 在 Qwen3-TTS 中是 **25Hz 路径中 token → Mel 的模型**。下图重点展示它对 LM、BigVGAN、Flow Matching 目标的依赖关系，以及滑动窗口块注意力的工作方式。

![[2026-04-18 09.54.34DiT 在 Qwen3-TTS.excalidraw|500]]

> [!important]
> 
> **关系要点**
> 
> - **DiT ← LM**：每收到 8 个新 token 触发一次 chunk 推理；需 16 tokens 缓冲等 look-ahead。
> 
> - **DiT → BigVGAN**：交接 Mel 频谱图而非波形，两者皆为因果流式设计。
> 
> - **DiT ↔ Flow Matching**：FM 是 DiT 的数学训练目标，具体推导见子页。

---

[[Flow Matching 数学推导与实现]]