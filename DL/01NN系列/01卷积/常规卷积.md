...
若使得输出维度减半，$\mathrm{padding}$ 的设置为：$\mathrm{padding}=\frac{\left(\text{output\_len}-1\right) \cdot s-\text{input\_len}+d\cdot(k-1)+1}{2}$，一般步长s取2，即$\mathrm{padding}=\frac{d\cdot(k-1)-1}{2}$
